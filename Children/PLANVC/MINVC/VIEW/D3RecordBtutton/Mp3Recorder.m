...
- (void)setRecorder
{
    ...
    //采样率
    [settings setValue :[NSNumber numberWithFloat:8000.0] forKey: AVSampleRateKey];//44100.0 11025.0
...
- (void)deleteArmCache{
    [self deleteFileWithPath:[self armPath]];
}
...
- (void)audio_PCMtoMP3{
    ...
    NSString *armFilePath = [self armPath];
...
//    }
//    NSData *curAudioData0 = [NSData dataWithContentsOfURL:[NSURL fileURLWithPath:cafFilePath]];
    NSData *curAudioData = EncodeWAVEToAMR([NSData dataWithContentsOfURL:[NSURL fileURLWithPath:cafFilePath]], 1, 16);
    BOOL ist = [curAudioData writeToFile:armFilePath atomically:YES];
    [self deleteCafCache];
    NSLog(@"arm转换结束");
    if (_delegate && [_delegate respondsToSelector:@selector(endConvertWithData:)] && ist) {
        NSData *voiceData = [NSData dataWithContentsOfFile:[self armPath]];
...
- (NSString *)armPath
{
    NSString *armPath = [NSTemporaryDirectory() stringByAppendingPathComponent:@"armRecord.arm"];
    return armPath;
}
