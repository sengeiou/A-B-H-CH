...
- (void)BreakUpAarmValue:(NSDictionary *)dic{
    if (dic) {
        NSArray *cmdValues = [dic[@"CmdValue"] componentsSeparatedByString:@","];
        if (cmdValues.count <= 1) return;
        for (int i = 0; i < [cmdValues[1] intValue]; i ++) {
            CHCmdClassDemo *mode = [[CHCmdClassDemo alloc] init];
            if (cmdValues.count <= (2 + i * 3)) return;
            mode.starTime = cmdValues[2 + i * 3];
            if (cmdValues.count <= (3 + i * 3)) return;
            mode.week = cmdValues[3 + i * 3];
            if (cmdValues.count <= (4 + i * 3)) return;
            mode.open = cmdValues[4 + i * 3];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell selectSwitchCallBack:^(UISwitch *oSwitch) {
        ...
        mode.open = [NSString stringWithFormat:@"%d",oSwitch.on];
        [selfWeak addAlarm:mode cell:indexPath delete:NO];
    }];
