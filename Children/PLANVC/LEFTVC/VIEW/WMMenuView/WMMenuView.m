...
- (void)addItems {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        if (i < self.titlesCount - 1) {
            CGRect frame1 = [self.frames[i + 1] CGRectValue];
            UILabel *lineLab = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(frame) + (CGRectGetMinX(frame1) - CGRectGetMaxX(frame))/2, 8, 1.5, frame.size.height - 16)];
